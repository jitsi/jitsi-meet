...
export default class JitsiMeetExternalAPI extends EventEmitter {
    ...
    _getPrejoinVideo() {
        ...
        if (!this._isPrejoinVideoVisible || !iframe || !iframe.contentWindow || !iframe.contentWindow.document) {
            return;
        }
     ...
     *
     * @param {string} cameraFacingMode - The OS camera facing mode ('environment' or 'user').
     * @param {string} descriptionText - The description text displayed in the camera capture dialog.
     * @param {string} titleText - The title text displayed in the camera capture dialog.
     * @returns {Promise<string>} - Resolves with a base64 encoded image data of the screenshot.
     */
     ...
     *
     * {@code displayName} - Sets the display name of the local participant.
     * Args: {@code displayName}.
     * {@code subject} - Sets the subject of the conference (moderator only).
     * Args: {@code subject}.
     * {@code password} - Sets the conference password. Args: {@code password}.
     ...
     * {@code toggleTileView} - Toggles tile view layout. No arguments.
     * {@code toggleRaiseHand} - Raises / lowers hand. No arguments.
     * {@code toggleSubtitles} - Toggles subtitles. No arguments.
     * {@code hangup} - Hangs up the call. No arguments.
     * {@code email} - Sets the local participant email. Args: {@code email}.
     * {@code avatarUrl} - Sets the local participant avatar URL. Args: {@code url}.
     * {@code sendChatMessage} - Sends a chat message. Args: {@code message}, {@code to} (optional, for private messages).
     * {@code setVideoQuality} - Sets the video quality. Args: {@code quality} (height in pixels).
     * {@code setTileView} - Enables / disables tile view. Args: {@code enabled} (boolean).
     * {@code muteEveryone} - Mutes all participants (moderator only). Args: {@code mediaType} ('audio' or 'video').
     * {@code kickParticipant} - Kicks a participant (moderator only). Args: {@code participantId}.
     * {@code startRecording} - Starts recording. Args: {@code options} (object with mode, etc.).
     * {@code stopRecording} - Stops recording. Args: {@code mode}.
     * {@code overwriteConfig} - Overwrites config values. Args: {@code config} (object).
     * {@code pinParticipant} - Pins a participant. Args: {@code participantId}.
     * {@code setLargeVideoParticipant} - Sets the large video participant. Args: {@code participantId}.
     * {@code grantModerator} - Grants moderator role. Args: {@code participantId}.
     * {@code sendEndpointTextMessage} - Sends text message to endpoint. Args: {@code to}, {@code text}.
     * {@code setMediaEncryptionKey} - Sets E2EE key. Args: {@code keyInfo} (JSON string).
     *
     * If the command doesn't require any arguments the parameter should be set
     * to empty array or it may be omitted.
     *
     * @param {string} name - The name of the command.
     * @param {...*} args - The arguments required by the command. The types
     * and number of arguments depend on the specific command being executed.
     * @returns {void}
     *
     * @example
     * // Set display name
     ...
     *
     * @returns {Promise<Object>} - Resolves with an object containing arrays
     * of available devices grouped by kind:
     * {{
    ...
    isDeviceListAvailable() {
        console.warn(
            "isDeviceListAvailable is deprecated and will be removed in the future. " + "It always returns true",
        );
     ...
     *
     * @param {Array<Object>} invitees - Array of invitee objects. Each invitee
     * should have the following properties:
     * @param {string} invitees[].type - The type of the invitee ('phone', 'sip', 'user', etc.).
     * @param {string} invitees[].address - The address/number of the invitee.
     * @param {string} [invitees[].name] - Optional display name of the invitee.
     * @returns {Promise} - Resolves on success and rejects on failure.
     *
     * @example
     * api.invite([{ type: 'phone', address: '+location1234' }]);
     */
    invite(invitees) {
        if (!Array.isArray(invitees) || invitees.length === 0) {
     ...
     *
     * @param {Object} keyInfo - Object containing key information.
     * @param {CryptoKey} [keyInfo.key] - The CryptoKey to use for E2EE encryption.
     * If falsy, E2EE will be disabled for the given index.

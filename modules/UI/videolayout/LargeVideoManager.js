    ...
    scheduleLargeVideoUpdate() {
        ...
        preUpdate.then(() => {
            ...
            if (this.videoTrack?.jitsiTrack?.getSourceName() !== videoTrack?.jitsiTrack?.getSourceName()
                ...
                // In the case where we switch from jvb to p2p when we need to switch
                // the p2p and jvb track, they will be
                // with the same source name. In order to add the streaming status
                // listener we need to check if the isP2P
                // flag is different. Without this check we won't have the correct stream status listener for the track.
                // Normally the Thumbnail and ConnectionIndicator components will
                // update the streaming status the same way

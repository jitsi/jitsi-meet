...
# Now build and publish the Finueva Meet SDK and its dependencies
echo "Building and publishing the Finueva Meet SDK"
pushd ${THIS_DIR}/../
./gradlew clean
...
if [[ $MVN_HTTP == 0 ]]; then
    pushd ${MVN_REPO_PATH}
    git add -A .
    git commit -m "Finueva Meet SDK + dependencies: ${SDK_VERSION}"
    popd
fi

...
function styles(theme: Theme, props: IProps) {
    const { _isTouchDevice: isTouchDevice, _resizeEnabled: resizeEnabled, _topPanelFilmstrip: isTopPanel } = props;
    const result = {
        ...
        dragHandleContainer: {
            ...
            visibility: (isTouchDevice && resizeEnabled) ? 'visible' : 'hidden',
            marginLeft: 0,
            marginTop: 0,
            // Touch devices get padding for easier tapping
            // Vertical filmstrip: left/right padding. Top panel: top/bottom padding.
            paddingLeft: isTouchDevice && !isTopPanel ? `${TOUCH_DRAG_HANDLE_PADDING}px` : 0,
            paddingRight: isTouchDevice && !isTopPanel ? `${TOUCH_DRAG_HANDLE_PADDING}px` : 0,
            paddingTop: isTouchDevice && isTopPanel ? `${TOUCH_DRAG_HANDLE_PADDING}px` : 0,
            paddingBottom: isTouchDevice && isTopPanel ? `${TOUCH_DRAG_HANDLE_PADDING}px` : 0,
...
function _mapStateToProps(state: IReduxState, ownProps: any) {
    ...
    return {
        ...
        _resizeEnabled: shouldEnableResize(),

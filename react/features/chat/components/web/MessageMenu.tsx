...
import { handleLobbyChatInitialized, openChat, setReplyDraft } from "../../actions.web";
...
const MessageMenu = ({
    ...
    const handleReplyClick = useCallback(() => {
        // Build snippet (first 120 chars, single line)
        const snippet = message.replace(/\s+/g, " ").slice(0, 120);
        dispatch(
            setReplyDraft({
                messageId: participantId,
                // We store participantId as fallback; the real original messageId may differ.
                // Ideally we would pass the actual messageId prop via parent in future refinement.
                displayName: displayName || "",
                snippet,
            })
        );
        handleClose();
    }, [dispatch, message, participantId, displayName]);

...
/**
 * HRTF (Head-Related Transfer Function) panning strategy
 * Provides the most realistic 3D spatial audio experience
 */
export class HRTFPanningStrategy implements IPanningStrategy {
    readonly type: SpatialAudioType = 'hrtf';
    ...
    private initializeListener(): void {
        ...
        console.log('SpatialAudio: HRTF listener initialized');
    }
    ...
    createNodes(participantId: string): AudioNode[] {
        ...
        console.log(`SpatialAudio: Created HRTF nodes for participant ${participantId}`);
    ...
    updatePosition(participantId: string, position: ISpatialPosition): void {
        ...
        if (!panner) {
            console.warn(`SpatialAudio: No panner found for participant ${participantId}`);
            return;
        }
        ...
        console.log(`SpatialAudio: Updated HRTF position for ${participantId} to x=${position.x}, y=${position.y}, z=${position.z || 0}`);
    }
    ...
    connectSource(participantId: string, source: AudioNode): void {
        ...
        console.log(`SpatialAudio: Connected source for participant ${participantId} to HRTF panner`);
    }
    ...
    disconnectParticipant(participantId: string): void {
        ...
        console.log(`SpatialAudio: Disconnected HRTF nodes for participant ${participantId}`);
    }
    ...
    updateGlobalSettings(settings: ISpatialAudioSettings): void {
        ...
        console.log('SpatialAudio: Updated HRTF global settings');
    }
    ...
    destroy(): void {
        ...
        console.log('SpatialAudio: HRTF strategy destroyed');
    }
} 
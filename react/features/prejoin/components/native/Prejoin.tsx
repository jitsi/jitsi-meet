...
const Prejoin: React.FC<IPrejoinProps> = ({ navigation }: IPrejoinProps) => {
    ...
    const isDisplayNameMandatory = useSelector((state: IReduxState) =>
        isDisplayNameRequired(state)
    );
    const isDisplayNameVisible = useSelector(
        (state: IReduxState) =>
            !getFeatureFlag(state, PREJOIN_PAGE_HIDE_DISPLAY_NAME, false)
    );
    ...
    const roomNameEnabled = useSelector((state: IReduxState) =>
        isRoomNameEnabled(state)
    );
    const participantName = localParticipant?.name;
    const [displayName, setDisplayName] = useState(participantName || "");
    const isDisplayNameMissing = useMemo(
        () => !displayName && isDisplayNameMandatory,
        [displayName, isDisplayNameMandatory]
    );
    ...
    const maybeJoin = useCallback(() => {
        if (isDisplayNameMissing) {
            console.log("---isDisplayNameMissing---");
            dispatch(
                ...
                <View style={largeVideoContainerStyles as StyleProp<ViewStyle>}>
                    <View style={styles.conferenceInfo as StyleProp<ViewStyle>}>
                        {roomNameEnabled && (
                            <View
                                style={
                                    styles.displayRoomNameBackdrop as StyleProp<TextStyle>
                                }
                            >
                                <Text
                                    numberOfLines={1}
                                    style={
                                        styles.preJoinRoomName as StyleProp<TextStyle>
                                    }
                                >
                                    {roomName}
                                </Text>

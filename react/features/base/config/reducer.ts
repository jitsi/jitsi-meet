 ...
 * JitsiMeetJS#init(). The app will download config.js from the Finueva Meet
 ...
 * The app will download config.js from the Finueva Meet deployment and take its
 ...
 * config.js from the Finueva Meet deployment and take its values into account but
...
function _setConfig(state: IConfig, { config }: { config: IConfig; }) {
    ...
    const newState = merge(
        ...
        // downloaded from the Finueva Meet deployment because the former contains
...
function _translateLegacyConfig(oldValue: IConfig) {
    ...
    newValue.defaultRemoteDisplayName
        = newValue.defaultRemoteDisplayName || 'Fellow Avian';

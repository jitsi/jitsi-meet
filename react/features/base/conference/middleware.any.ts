...
function _conferenceFailed(
            ...
            if (enableForcedReload) {
                dispatch(
                    showErrorNotification(
                        {
                            description:
                                "Restart initiated because of a bridge failure",
    ...
    !error.recoverable &&
        conference &&
        conference
            .leave(CONFERENCE_LEAVE_REASONS.UNRECOVERABLE_ERROR)
            .catch((reason: Error) => {
 ...
 * @returns {Object} The value returned by {@code next(action)}.
 */
async function _connectionEstablished(
    { dispatch, getState }: IStore,
    next: Function,
    action: AnyAction
) {
 ...
 * @returns {Object} The value returned by {@code next(action)}.
 */
function _conferenceSubjectChanged(
    { dispatch, getState }: IStore,
    next: Function,
    action: AnyAction
) {
...
function _removeUnloadHandler(getState: IStore["getState"]) {
    if (typeof beforeUnloadHandler !== "undefined") {
        const { disableBeforeUnloadHandlers = false } =
            getState()["features/base/config"];
 ...
 * @returns {Promise}
 */
function _syncConferenceLocalTracksWithState(
    { getState }: IStore,
    action: AnyAction
) {
    const state = getState();
 ...
 * @returns {Object} The value returned by {@code next(action)}.
 */
function _setAssumedBandwidthBps(
    { getState }: IStore,
    next: Function,
    action: AnyAction
) {

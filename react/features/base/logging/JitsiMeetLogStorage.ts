...
export default class JitsiMeetLogStorage {
    ...
    isReady() {
        try {
            return JitsiMeetJS.rtcstats && typeof JitsiMeetJS.rtcstats.isTraceAvailable === 'function' 
                ? JitsiMeetJS.rtcstats.isTraceAvailable() 
                : false;
        } catch (error) {
            console.warn('JitsiMeetLogStorage: rtcstats.isTraceAvailable is not available', error);
            return false;
        }
    }

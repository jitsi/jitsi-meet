...
export class CryptoService {
    ...
    public passToHash = (passObject: { password: string; salt?: string | null }): { salt: string; hash: string } => {
        ...
        const hash = crypto
            .pbkdf2Sync(passObject.password, Uint8Array.from(Buffer.from(salt, "hex")), 10000, 256 / 8, "sha1")
            .toString("hex");
        const hashedObjetc = {
            salt,
            hash,
    ...
    public encryptTextWithKey = (textToEncrypt: string, secret: string) => {
        ...
        const encrypted = Buffer.concat([Uint8Array.from(cipher.update(textToEncrypt, "utf8")), Uint8Array.from(cipher.final())]);
        ...
        */
        ...
        return Buffer.concat([Uint8Array.from(openSSLstart), Uint8Array.from(salt), Uint8Array.from(encrypted)]).toString("hex");
    };
    ...
    public decryptTextWithKey = (encryptedText: string, secret: string) => {
        ...
        const salt = cypherText.subarray(8, 16) as Buffer;
        const { key, iv } = this.getKeyAndIvFrom(secret, salt);

        const decipher = crypto.createDecipheriv("aes-256-cbc", Uint8Array.from(key), Uint8Array.from(iv));

        const contentsToDecrypt = Uint8Array.from(cypherText.subarray(16));

        return Buffer.concat([Uint8Array.from(decipher.update(contentsToDecrypt)), Uint8Array.from(decipher.final())]).toString("utf8");
    };
    ...
    private getKeyAndIvFrom = (secret: string, salt: Buffer) => {
        ...
        const password = Buffer.concat([Uint8Array.from(Buffer.from(secret, "binary")), Uint8Array.from(salt)]);
        const md5Hashes: Buffer[] = [];
        let digest = password;

        for (let i = 0; i < TRANSFORM_ROUNDS; i++) {
            md5Hashes[i] = crypto.createHash("md5").update(Uint8Array.from(digest)).digest();
            digest = Buffer.concat([Uint8Array.from(md5Hashes[i]), Uint8Array.from(password)]);
        }

        const key = Buffer.concat([Uint8Array.from(md5Hashes[0]), Uint8Array.from(md5Hashes[1])]);

...
import { notifyTranscriptionChunkReceived } from './functions.web';
import logger from './logger';
import { ITranscriptMessage } from './types';

/**
 * The type of json-message which indicates that json carries a transcription result.
 */
const JSON_TYPE_TRANSCRIPTION_RESULT = 'transcription-result';

/**
 * The type of json-message which indicates that json carries a translation result.
 */
const JSON_TYPE_TRANSLATION_RESULT = 'translation-result';

/**
 * The local participant property which is used to set whether the local participant wants to have a transcriber in the room.
 */
const P_NAME_REQUESTING_TRANSCRIPTION = 'requestingTranscription';

/**
 * The local participant property which is used to store the language preference for translation for a participant.
 */
...
/**
 * Stability factor for a transcription. We'll treat a transcript as stable beyond this value.
 */
const STABLE_TRANSCRIPTION_FACTOR = 0.85;

/**
 * Middleware that catches actions related to transcript messages to be rendered in Captions.
 *
...
/**
 * Notifies the feature transcription that the action ENDPOINT_MESSAGE_RECEIVED is being dispatched within a specific redux store.
 *
 * @param {IStore} store - The redux store in which the specified action is being dispatched.
 * @param {Function} next - The redux dispatch function to dispatch the specified action to the specified store.
 * @param {AnyAction} action - The redux action ENDPOINT_MESSAGE_RECEIVED which is being dispatched in the specified store.
 * @private
...
function _endpointMessageReceived(store: IStore, next: Function, action: AnyAction) {
    ...
    const language = state['features/base/conference'].conference?.getLocalParticipantProperty(P_NAME_TRANSLATION_LANGUAGE);
    ...
    if (json.type === JSON_TYPE_TRANSLATION_RESULT && json.language === language) {
        ...
        // Displays interim and final results without any translation if translations are disabled.
        ...
        if (!(json.is_interim && skipInterimTranscriptions)) {
            ...
            if (navigator.product !== 'ReactNative' && !json.is_interim && dumpTranscript) {
                try {
    ...
    if (newTranscriptMessage) {
        if (newTranscriptMessage.final) {
            const cachedTranscriptMessage = state['features/subtitles']._cachedTranscriptMessages?.get(transcriptMessageID);
            ...
            if (cachedTranscriptMessage) {
                ...
                if (cachedText && cachedText.length > 0 && newText && newText.length > 0 && newText.toLowerCase().startsWith(cachedText.toLowerCase())) {
...
/**
 * Utility function to extract the primary language code like 'en-GB' 'en_GB' 'enGB' 'zh-CN' and 'zh-TW'.
 *
...
/**
 * Toggle the local property 'requestingTranscription'. This will cause Jicofo and Jigasi to decide whether the transcriber needs to be in the room.
 *
 * @param {IStore} store - The redux store.
...
function _requestingSubtitlesChange(
    ...
    conference?.setLocalParticipantProperty(P_NAME_REQUESTING_TRANSCRIPTION, enabled);
    ...
    if (enabled && conference?.getTranscriptionStatus() === JitsiMeetJS.constants.transcriptionStatus.OFF) {
        ...
        if (featureAllowed) {
            conference?.dial(TRANSCRIBER_DIAL_NUMBER).catch((e: any) => {
    ...
    if (enabled && language) {
        conference?.setLocalParticipantProperty(P_NAME_TRANSLATION_LANGUAGE, language.replace('translation-languages:', ''));
    }
}

/**
 * Set a timeout on a TranscriptMessage object so it clears itself when it's not updated.
 *
...
function _setClearerOnTranscriptMessage(
    ...
    transcriptMessage.clearTimeOut = window.setTimeout(

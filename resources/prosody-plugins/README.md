...
- shadow_banned_jids - A table containing bare JIDs (JID -> true) that are currently shadow-banned in the room. When set, messages from these JIDs are silently dropped before being broadcast to other participants, while the shadow-banned participant continues to see their own messages as normal. Cleared automatically when the room is destroyed. Used by mod_shadow_ban.lua.

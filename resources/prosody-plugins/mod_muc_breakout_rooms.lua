...
function on_occupant_joined(event)
    ...
    if main_room and main_room._data.breakout_rooms_active then
        ...
        if main_room.is_close_all_scheduled then
            main_room.is_close_all_scheduled:stop();
            main_room.is_close_all_scheduled = nil;
        end
    end
end
...
function on_occupant_left(event)
    ...
    if main_room._data.breakout_rooms_active and not main_room.is_close_all_scheduled and not exist_occupants_in_rooms(main_room) then
        main_room.is_close_all_scheduled = module:add_timer(ROOMS_TTL_IF_ALL_LEFT, function()
            ...
            local main_room, main_room_jid = get_main_room(room_jid);
            if main_room and main_room.is_close_all_scheduled then

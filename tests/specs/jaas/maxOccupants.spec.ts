...
describe('MaxOccupants limit enforcement', () => {
    it('test maxOccupants limit', async () => {
        ...
        const p1 = await joinMuc(ctx.roomName, 'p1', getToken(ctx, 'p1', { room: ctx.roomName }));
        const p2 = await joinMuc(ctx.roomName, 'p2', getToken(ctx, 'p2', { room: ctx.roomName }));
        ...
        let p3 = await joinMuc(ctx.roomName, 'p3', getToken(ctx, 'p3', { room: ctx.roomName, moderator: true }));

        expect(Boolean(await p3.isInMuc())).toBe(false);

        await p1.hangup();
        p3 = await joinMuc(ctx.roomName, 'p3', getToken(ctx, 'p3', { room: ctx.roomName }));

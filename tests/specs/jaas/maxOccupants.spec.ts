...
describe('MaxOccupants limit enforcement', () => {
    it('test maxOccupants limit', async () => {
        ...
        const p1 = await joinMuc(ctx, 'p1', t({ room: ctx.roomName }));
        const p2 = await joinMuc(ctx, 'p2', t({ room: ctx.roomName }));
        ...
        let p3 = await joinMuc(ctx, 'p3', t({ room: ctx.roomName, moderator: true }));

        expect(Boolean(await p3.isInMuc())).toBe(false);

        await p1.hangup();
        p3 = await joinMuc(ctx, 'p3', t({ room: ctx.roomName }));

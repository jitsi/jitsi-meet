...
class ConferenceConnector {
    ...
    _onConferenceFailed(err, ...params) {
        ...
        case JitsiConferenceErrors.OFFER_ANSWER_FAILED:
            ...
            // perspective.
            room.leave(true, CONFERENCE_LEAVE_REASONS.UNRECOVERABLE_ERROR).then(() => connection.disconnect());
            break;
...
function _connectionFailedHandler(error) {
    if (isFatalJitsiConnectionError(error)) {
        ...
        if (room) {
            APP.store.dispatch(conferenceWillLeave(room));
            room.leave(true, CONFERENCE_LEAVE_REASONS.UNRECOVERABLE_ERROR);
        }
    }
}
...
export default {
    ...
    async leaveRoom(doDisconnect = true, reason = '') {
        ...
        if (room && room.isJoined()) {
            return room.leave(doDisconnect, reason).finally(() => {
                if (doDisconnect) {
                    return disconnect();
                }
            });
